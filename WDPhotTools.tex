...
The catalogue available form GF21 has presented an
excellent sample of WDs for comparison. The models available online is slightly
different from the set available from their work~(priv. comm.). Furthermore,
we are correcting for the interstellar reddening at the instantaneous
temperature in each step of the fit, as opposed to using the interpolated
values for a $7000$\,K source from \citet{2011ApJ...737..103S}. Therefore,
some scattering is expected when the fitted values are compared against
each other. The solutions are fitted with the three Gaia filters,
distance, and reddening with using the catalogued value as provided from
their work. The distribution of the fitted effective
temperatures~(Fig.~\ref{fig:comparison}) are aligned with the identity line
with a Pearson correlation coefficient of 0.817 and 0.884 for the subset with
$10\sigma$ parallax.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{comparison_placeholder.png}
    \caption{Comparison of the effective temperature fitting against GF21. The blue scattered points show all the fitted
    data, while the orange shows the subset with $10\sigma$ parallax.}
